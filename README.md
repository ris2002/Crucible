...
| Suite | Tests | Focus |
|---|---|---|
| `test_ai_output_eval.py` | 6 | Golden-dataset schema validation for `parse_forge_ready` (title/summary/tags extraction); regression guard on safety-critical system prompt clauses |
| `test_crisis_guardrails.py` | 16 | 9 adversarial phrasings that must trigger the crisis response, 7 benign phrasings that must not over-trigger |
| `test_ai_router.py` | 9 | Provider dispatch (Anthropic/OpenAI/Google), fallback-to-Anthropic on an unrecognized provider, error propagation on provider failure |
| `test_cost_controls.py` | 3 | Tool-use loop cap (`max_iterations=8`), token usage aggregation across iterations, per-request budget assertion |

### Correction found via testing
